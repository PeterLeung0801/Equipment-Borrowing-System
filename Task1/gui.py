...
# If loading existing IDs from file, adjust the counter
if manager.records:
    last = max(int(r.id[1:]) for r in manager.records if r.id.startswith("B"))
    manager._id_counter = iter(range(last + 1, 9999))

def refresh_list():
    listbox.delete(0, tk.END)
    for r in manager.sorted_by_return():
        listbox.insert(tk.END, str(r))
    update_stats()

def update_stats():
    borrow_lines, upcoming, overdue = manager.stats()
    stats_text.delete(1.0, tk.END)
    if borrow_lines:
        stats_text.insert(tk.END, "目前借出狀況：\n" + "\n".join("  " + line for line in borrow_lines) + "\n\n")
    if overdue:
        stats_text.insert(tk.END, "逾期未還：\n" + "\n".join("  " + str(r) for r in overdue) + "\n\n")
    if upcoming:
        stats_text.insert(tk.END, "即將到期（3天內）：\n" + "\n".join("  " + str(r) for r in upcoming))
...
def add_borrow():
    ...
    # Admin name is stored but not used in display (could extend model if needed)
    # To properly store, add __admin_name attribute to the model
